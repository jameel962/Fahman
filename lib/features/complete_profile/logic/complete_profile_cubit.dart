...
class CompleteProfileCubit extends Cubit<CompleteProfileState> {
  ...
  Future<bool> submit() async {
    ...
    if (avatar == null || avatar.isEmpty) {
      emit(state.copyWith(error: 'الرجاء اختيار صورة ملف شخصي'));
      return false;
    }

    // Validate name
    if (name.isEmpty) {
      emit(state.copyWith(error: 'الرجاء إدخال الاسم'));
      return false;
    }

    if (name.length < 6) {
      emit(state.copyWith(error: 'الاسم يجب أن يكون 6 أحرف على الأقل'));
      return false;
    }
    ...
    if (!nameRegex.hasMatch(name)) {
      emit(state.copyWith(error: 'الاسم يجب أن يحتوي على حروف فقط'));
      return false;
    }

    // Validate phone number
    if (phone.isEmpty) {
      emit(state.copyWith(error: 'الرجاء إدخال رقم الهاتف'));
      return false;
    }
    ...
    if (!jordanReg.hasMatch(localPhone)) {
      emit(
        state.copyWith(
          error:
              'الرجاء إدخال رقم جوال أردني صالح يبدأ بـ 077 أو 078 أو 079 ويتكون من 10 أرقام',
        ),
      );
      return false;
    }
